...
public class FormNhanVien extends javax.swing.JPanel {
    ...
    public FormNhanVien() {
        initComponents();
        String[] cols  ={"Mã nhân viên", "Tên nhân viên", "Địa chỉ", "SDT"};
        model = new DefaultTableModel(cols, 0);
        jTable1.setModel(model);
        add= false;
        jButton1.setEnabled(add);
        file = "src/main/java/controller/NV.txt";
        check();
    }
    private void check(){
        File f = new File(file);
        if(f.exists()){
            List<Nhanvien> list = IOFile.doc(file);
            if(list.size()> 0){
        ...
        if(add){
            ...
            try {
                if(ten.length() == 0){
                    ...
                    throw new Exception("Ten khong duoc chua li tu la!");
                }
            } catch (Exception e) {
                isOk = false;
                e.printStackTrace();
            ...
            if(isOk){
                mh = new Nhanvien(ma, ten, diaChi, sdt);
